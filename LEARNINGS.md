...
### `schemaMarkup` prefer-first pattern for `application/ld+json`

When a Sanity document carries a stored `schemaMarkup` JSON string (e.g. written by PublishForge), check and emit it before any generated-schema logic. Wrap in try/catch so malformed JSON silently falls through to the generated path.
...
Applies universally: any document type that gains `schemaMarkup` field gets stored-schema preference automatically, with no further changes to `addSEOJsonLd()`.

Source: T-004. See `components/SEO.tsx:addSEOJsonLd`.

---

### GROQ `...,` wildcard covers all fields including arrays of objects

`...,` at document root in a GROQ projection captures every field — scalars, arrays, and arrays of objects — with full content. Explicit projections like `faqItems[] { ..., }` are only needed when reshaping (e.g. computed fields, deep reference expansion with `->`, renaming). Do not add them for plain pass-through.
...
### Verify Edit tool writes with `git diff` after context compression

After a long session where the context window has been compressed, Edit tool success messages may not reflect actual disk state. Always run `git diff -- <file>` after critical edits in multi-stage workflows to confirm the change landed on disk.

Source: T-004 (query.ts edit reported success but showed no diff during the following test stage).
