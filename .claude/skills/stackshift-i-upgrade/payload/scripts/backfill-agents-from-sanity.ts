...
const sinceDate = args.since as string | undefined;
const wikiDir =
...
async function main() {
  ...
  if (docType === "all" || docType === "post") {
    const sinceFilter = sinceDate ? ` && publishedAt >= $since` : "";
    const postQuery = filterSlug
      ? `*[_type == "post" && slug.current == $slug && !(_id in path("drafts.**"))${sinceFilter}] {
          _id, title, "slug": slug.current, excerpt, publishedAt, body,
          "categories": categories[]->.title,
          "seo": seo { seoTitle, seoDescription }
        }`
      : `*[_type == "post" && defined(slug.current) && !(_id in path("drafts.**"))${sinceFilter}] {
          _id, title, "slug": slug.current, excerpt, publishedAt, body,
          "categories": categories[]->.title,
          "seo": seo { seoTitle, seoDescription }
        }`;
    ...
    console.log(`[backfill] fetched ${posts.length} posts${sinceDate ? ` (since ${sinceDate})` : ""}`);
  }

  if (docType === "all" || docType === "page") {
    const pageSinceFilter = sinceDate ? ` && publishDateTime >= $since` : "";
    const pageSectionsProjection = `"sections": sections[]->{ _type, label, variant, variants }`;
    const pageQuery = filterSlug
      ? `*[_type == "page" && slug.current == $slug && !(_id in path("drafts.**"))${pageSinceFilter}] {
          _id, title, "slug": slug.current, publishDateTime,
          ${pageSectionsProjection},
          "seo": seo { seoTitle, seoDescription }
        }`
      : `*[_type == "page" && defined(slug.current) && !(_id in path("drafts.**")) && slug.current != "home"${pageSinceFilter}] {
          _id, title, "slug": slug.current, publishDateTime,
          ${pageSectionsProjection},
          "seo": seo { seoTitle, seoDescription }
        }`;
    ...
    console.log(`[backfill] fetched ${pages.length} pages${sinceDate ? ` (since ${sinceDate})` : ""}`);
  }
