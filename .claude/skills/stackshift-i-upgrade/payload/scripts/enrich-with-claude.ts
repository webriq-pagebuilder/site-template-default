...
- faq: array of {question, answer} pairs (max 5, only if genuinely FAQ-worthy content exists)
...
export async function enrichFrontmatter(
  ...
  try {
    const response = await client.messages.create({
      // Headroom for models that run adaptive thinking by default (e.g.
      // claude-sonnet-5): thinking tokens share max_tokens, so 1024 risked
      // truncating the JSON answer. The enrichment payload itself is small.
      model,
      max_tokens: 8192,
    ...
    // Find the text block rather than assuming content[0]. Models with adaptive
    // thinking on (e.g. claude-sonnet-5, where it defaults on) emit a leading
    // thinking block before the text, so content[0] is not necessarily the answer.
    const block = response.content.find((b) => b.type === "text");
    if (!block || block.type !== "text") {
      console.warn(
        "[enrich-with-claude] no text block in response; blocks:",
    ...
    if (Array.isArray(parsed.faq) && parsed.faq.length > 0) {
      result.faq = parsed.faq.map((item: any) => ({
