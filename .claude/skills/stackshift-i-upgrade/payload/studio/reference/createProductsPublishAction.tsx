...
import { NEXT_PUBLIC_SANITY_STUDIO_IN_CSTUDIO, PUBLISHFORGE_WEBHOOK_URL } from "studio/config";
...
export default function createProductsPublishAction(props) {
  ...
  return {
    ...
    onHandle: async () => {
      ...
      // Fires for page and post documents. Routes through the local proxy to
      // avoid CORS — Sanity Studio runs in the browser and cannot POST directly
      // to an external Vercel URL.
      // Fire-and-forget: failure must not block the publish.
      if (
        (props.type === "page" || props.type === "post") &&
