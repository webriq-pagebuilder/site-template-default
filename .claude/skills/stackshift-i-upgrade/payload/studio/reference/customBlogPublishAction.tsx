// Custom Publish Action for Blog Posts
...
import { SCHEDULED_PUBLISHING_URL, SUPABASE_ANON_KEY, PUBLISHFORGE_WEBHOOK_URL } from "studio/config";
  ...
  return {
    ...
    onHandle: async () => {
      ...
      try {
        ...
        const liveEditPublishDate = props.draft?.publishedAt;
        ...
        await client
          .patch(props.id, {
            set: {
              // if a publish datetime exists in draft, else fallback with current date on publish
              publishedAt: liveEditPublishDate ?? currentDateTime,
            },
            unset: ["publishStatus", "scheduledId"],
          })
          .commit();

        // Notify PublishForge to generate agent content for this blog post.
        // Fire-and-forget: agent generation failure must not block the publish.
        // Uses a local proxy to avoid CORS issues when Studio runs in the browser.
        if (PUBLISHFORGE_WEBHOOK_URL) {
          fetch("/api/publishforge-proxy", {
            ...
            body: JSON.stringify({
              ...
              slug:
                props.draft?.slug?.current ?? props.published?.slug?.current,
            }),
          }).catch((err) => {
