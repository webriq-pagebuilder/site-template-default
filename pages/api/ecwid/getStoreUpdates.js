...
export default async (req, res) => {
  ...
  try {
          ...
          if (studioBundleProducts.length > 0) {
            ...
            const getproductBySku = await Promise.all(
              getProductSku.map(async (prod) => {
                return await fetch(
                  baseUrl + `/${storeId}/products?sku=${prod?.sku.toString()}`,
                  {
                    method: "GET",
                    headers: requestHeaders,
                  }
    ...
    // Send email notification that this endpoint failed
    // await fetch(`${siteUrl}/api/email/productBundleNotification`, {
    //   method: "POST",
    //   headers: {
    ...
    //   },
    //   body: JSON.stringify({
    //     recepients: process.env.WEBRIQ_ADMIN_EMAIL,
    //     cc: process.env.WEBRIQ_ADMIN_EMAIL_CC,
    //   }),
    // }).then(() =>
    //   console.log(
    //     "[INFO] Successfully sent email to notify WebriQ team on issue"
    //   )
    // );
