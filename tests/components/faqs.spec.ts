...
async function createFaqsVariant(pageTitle, variantLabel, variantIndex) {
    ...
    if (variantIndex !== 1) {
        for(const faqs of faqsData) {
            ...
            await page.locator(`input[value^="${faqs.value}"]`).fill(faqs.updateQuestion);
        ...
        for(const faqs of faqsWithCategories) {
            ...
            if(Array.isArray(faqs.question)) {
                for (const question of faqs.question) {
                    ...
                    await page.locator(`input[value^="${question}"]`).fill(faqs.updatedQuestion);
                    await page.getByLabel('Add its answer').click();
                ...
                await page.locator(`input[value^="${faqs.question}"]`).fill(faqs.updatedQuestion);
                await page.getByLabel('Add its answer').click();
                await page.getByLabel('Add its answer').fill(faqs.updatedAnswer);
                await page.getByLabel('Close dialog').nth(1).click();
                await page.getByLabel('Close dialog').click();
    ...
    if (variantIndex === 0) {
        for (const faqs of faqsData) {
            ...
            await openUrlPage.getByPlaceholder('Search, find any question you').fill(faqs.updateQuestion);
...
test("Create Faqs Variant A", async () => {
    await createFaqsVariant("Faqs Page A", "Faqs New Page A", 0);
})

test("Create Faqs Variant B", async () => {
    await createFaqsVariant("Faqs Page B", "Faqs New Page B", 1);
})

test("Create Faqs Variant C", async () => {
    await createFaqsVariant("Faqs Page C", "Faqs New Page C", 2);
})
