...
export async function createNavigationVariant(
  pageTitle,
  variantLabel,
  variantIndex,
  isInternalLink
  ...
  if (variantIndex < 4) {
    ...
    const primaryButtonLabel = page
      .getByTestId("field-variants.primaryButton.label")
      .getByTestId("string-input");
    const primaryButtonLinkExternal = page
      .getByTestId("field-variants.primaryButton.linkExternal")
      .getByLabel("URL");
    const primaryButtonLinkTarget = page.getByTestId(
      "field-variants.primaryButton.linkTarget"
    );

    const secondaryButton = page.getByRole("button", {
      name: "Secondary Button",
    });
    const secondaryButtonLabel = page
      .getByTestId("field-variants.secondaryButton.label")
      .getByTestId("string-input");
    const secondaryButtonLinkExternal = page
      .getByTestId("field-variants.secondaryButton.linkExternal")
      .getByLabel("URL");
    const secondaryButtonLinkTarget = page.getByTestId(
      "field-variants.secondaryButton.linkTarget"
    );

    await primaryButton.click();
    await primaryButtonLabel.click();
    await primaryButtonLabel.fill(inputPrimaryButton);
    await expect(primaryButtonLabel.inputValue()).resolves.toBe(
      inputPrimaryButton
    );
    ...
    if (isInternalLink) {
      ...
      await expect(primaryButtonLinkExternal.inputValue()).resolves.toBe(
        externalLinkUrl
      );
      await primaryButtonLinkTarget
    ...
    await expect(secondaryButtonLabel.inputValue()).resolves.toBe(
    ...
    if (isInternalLink) {
      ...
      await page
        .getByRole("button", { name: "Thank you Published No" })
        .click();
      await secondaryButtonLinkTarget
        .getByText("Self (default) - open in the")
        .click();
    } else {
      await page
      ...
      await expect(secondaryButtonLinkExternal.inputValue()).resolves.toBe(
        externalLinkUrl
      );
      await secondaryButtonLinkTarget
        .getByText("Blank - open on a new tab (")
...
const createNavigationTest = async (
  pageTitle,
  variantName,
  variantIndex,
  isInternalLink,
...
const navigationVariants = [
  {
    ...
    pageTitle: "Navigation Variant A",
  ...
  },
  {
    ...
    pageTitle: "Navigation Variant B",
  ...
  },
  {
    ...
    pageTitle: "Navigation Variant C",
  ...
  },
  {
    ...
    pageTitle: "Navigation Variant D",
  ...
  },
  {
    ...
    pageTitle: "Navigation Variant E",
    variantLabel: "Navigation New Page E",
    variantIndex: 4,
    isInternalLink: false,
...
navigationVariants.forEach((variant) => {
