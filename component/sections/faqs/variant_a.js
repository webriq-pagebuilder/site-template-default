...
function VariantA({ subtitle, title, faqs }) {
  ...
  const searchedFAQs = !searchTerm 
    ...
    : data?.filter(items => items?.question?.toLowerCase().includes(searchTerm)); // get search results based on data
  
  // display FAQs 
  const FAQs = ({ items }) => {
    return <ul className="space-y-4 lg:space-y-6">
      {items?.map((faq, index) => (
          ...
          >
            ...
            >
              <path
                ...
                strokeWidth={2}
                d={!searchTerm ? faq?.hidden === false ? "M19 14l-7 7m0 0l-7-7m7 7V3" : "M5 10l7-7m0 0l7 7m-7-7v18" : "M19 14l-7 7m0 0l-7-7m7 7V3"}
              />
            </svg>
          </button>
          {!searchTerm ? (
            <p className={`${faq?.hidden === false ? "hidden" : null} mt-4 text-gray-400 font-normal leading-loose`}>
              {faq?.answer}
            </p>
          ): <p className="mt-4 text-gray-400 font-normal leading-loose">{faq?.answer}</p>}
        </li>
      ))}
    </ul>
  };
